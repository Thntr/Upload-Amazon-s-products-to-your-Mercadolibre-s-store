...
class megaPostTestSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        if tittle == []:
            print("YOU ARE TESTING THE PARSE AS AN INSTANCE OF THE CLASS")
            return self.parse(response)

        availability = response.css("#availability > span").css("::text").extract()
        js_prime = response.xpath("//script[contains(text(), '\"bbopRuleID\":\"Acquisition_AddToCart_PrimeBasicFreeTrialUpsellEligible\"')]/text()").extract()
        print("THE PRIME INFO EXTRACTED IS LIKE THIS: {}".format(js_prime))
        ...
        print("THE PRIME STATEMENT IS LIKE THIS: {}".format(prime))
        print("THE AVAILABILITY RESPONSE IS THIS: {}".format(availability))
        print("THIS IS THE PRICE: {}".format(price))
        print("THE DESCRIPTION LOOK LIKE THIS: {}".format(description))
        ...
        if picture == [] or picture == None:
            print("THE PICTURES HAS NOT EXTRACTED!")
            picture = response.css("#landingImage").css("::src").extract()
            pic_list = []
            for _ in range(5):
                pic_list.append(picture[0])
            picture = pic_list
        ...
        coolDict = {"amazonLink": amazonLink, "price": items["price"], "Prime?": items["prime"], "selledByAmazon": items["selledByAmazon"],
