...
class postTestSpider(scrapy.Spider):
    ...
    data = get_data("toysItems.json")
    newArray = []

    # Start a set of parallel publications
    print("¿What # of prompt is is this?'THIS IS THE POSTEST.PY!!!'\n")
    ...
    for i in range(int(listOfExtremes[beautifulNumber - 1]), int(listOfExtremes[beautifulNumber])):
        newArray.append(data["startUrl"][i])

    start_urls = newArray

    print(" ͡° ͜ʖ ͡°these are the amount of urls for the amazonPostSpider...")
    print(len(start_urls))
    print(" ͡° ͜ʖ ͡°")

    def parse(self, response):

        print(r"¯\\_(ツ)_/¯ POST TEST CRAWLER ¯\\_(ツ)_/¯")
        ...
        if items["availability"][0] == "Disponible." and items["prime"] == "Amazon Prime":
            ...
            if value != None and len(items["selledByAmazon"]) == 1 and pictureFunc:
                ...
                body.update(postSet(newTittle, "MLM157927", definitiveDescription))
        ...
        coolDict = {"amazonLink": amazonLink, "price": items["price"], "Prime?": items["prime"], "selledByAmazon": items["selledByAmazon"],
                    ...
                    "pictures": items["picture"], "body": body, "description": items["description"],
                    "meli": newMeli}
