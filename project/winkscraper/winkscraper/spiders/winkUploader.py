...
class megaPostTestSpider(scrapy.Spider):
    name = "winkUpload"
    data = get_data("winkSynchList.json")
    newArray = []
    ...
    def parse(self, response):
        ...
        try:
            print(r"¯\\_(ツ)_/¯ Wink POST CRAWLER ¯\\_(ツ)_/¯")
            ...
            definitiveDescription = definitiveDescription.replace("@","").replace("https://","").replace(".com","").replace(".net","").replace("www","").replace("http://","").replace("[","").replace("'","")
            definitiveDescription = "None"
        ...
        if self.counter >= self.noFUrls - 8:
            ...
            pathForStorageTheSynchItems = r"C:\Users\ramzhacker\Documents\MEGA\Newy\Wink_Inc\WinkAI\#WinkAIMLApp\pythonSdkMaster\winktech\project\winkscraper\winkscraper\spiders\itemsForSynch"
            synchLinks = get_json("synchAmazonList" + str(self.beautifulNumber) + ".json", path=pathForStorageTheSynchItems)

            pathStorageOfTheFailedItems = r"C:\Users\ramzhacker\Documents\MEGA\Newy\Wink_Inc\WinkAI\#WinkAIMLApp\pythonSdkMaster\winktech\project\winkscraper\winkscraper\spiders\failedItems"
            failedLinks = get_json("synchAmazonFails" + str(self.beautifulNumber) + ".json", path=pathStorageOfTheFailedItems)
            ...
            with open(failedLinks, "r+") as file:
                tmp = json.load(file)
                list0 = dfSynch["amazonFails"]

                tmp["amazonFails"] = self.amazonFails
                file.seek(0)
